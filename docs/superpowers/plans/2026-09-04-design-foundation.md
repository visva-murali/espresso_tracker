...
The final whole-branch review (2026-09-04) covered all 9 tasks together and found 15 items, from a critical validation gap (an empty ShotForm submit could persist a corrupt 0/0/0 shot) to smaller consistency, navigation, and doc-accuracy findings. All 15 were fixed in one combined fix-round commit. A scoped re-review of that fix round found all 15 addressed but flagged 2 issues the fix round itself introduced; one was accepted as-is with recorded reasoning (a very-low-probability legacy-data edge case, not a real dead end), the other (video-attach/delete failures blanking the whole shot detail page) was fixed directly in a follow-up commit. This branch is now ready for `superpowers:finishing-a-development-branch`.
